...
const pluginConfig = {
    name: 'tobotak',
    alias: ['botak', 'dibotakin'],
    category: 'ai',
    description: 'Ubah foto jadi botak',
    usage: '.tobotak (reply/caption foto)',
    example: '.tobotak',
    isOwner: false,
    isPremium: false,
    isGroup: false,
...
const TOBOTAK_URL = 'https://api-faa.my.id/faa/tobotak?url='
...
async function handler(m, { sock }) {
    ...
    if (!fromQuoted && !fromDirect) {
        return m.reply(
            `👨‍🦲 ᴛᴏ ʙᴏᴛᴀᴋ\n\n` +
            `> Kirim/reply foto yang mau diubah jadi botak\n\n` +
            `\`${m.prefix}tobotak\``
        )
    }
    ...
    try {
        ...
        const res = await axios.get(`${TOBOTAK_URL}${encodeURIComponent(imageUrl)}`, {
        ...
        await sock.sendMessage(m.chat, {
            image: resultBuffer,
            caption: 'ɪɴɪ ᴛᴜᴀɴ ᴀᴋᴜ ᴜᴅᴀʜ ᴍᴇᴍʙᴜᴀᴛ ꜰᴏᴛᴏ ᴍᴇɴᴊᴀᴅɪ ʙᴏᴛᴀᴋ 👨‍🦲'
        }, { quoted: m })
