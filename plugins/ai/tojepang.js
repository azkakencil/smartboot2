...
const pluginConfig = {
    name: 'tojepang',
    alias: ['jepang', 'japan', 'kawaii', 'wibu'],
    category: 'ai',
    description: 'Ubah foto kamu jadi ala orang Jepang atau karakter estetik',
    usage: '.tojepang (reply/caption foto)',
    example: '.tojepang',
    isOwner: false,
    isPremium: false,
    isGroup: false,
...
const TOJEPANG_URL = 'https://api-faa.my.id/faa/tojepang?url='
...
async function handler(m, { sock }) {
    ...
    if (!fromQuoted && !fromDirect) {
        return m.reply(
            `🌸 *ᴛᴏ ᴊᴇᴘᴀɴɢ* 🌸\n\n` +
            `> Mau liat muka Tuan pas berubah jadi cowok/cewek Jepang yang kawai? 🇯🇵\n` +
            `> Kirim/reply fotonya ke sini cepetan, Tuan! ✨\n\n` +
            `\`${m.prefix}tojepang\``
        )
    }
    ...
    try {
        ...
        const res = await axios.get(`${TOJEPANG_URL}${encodeURIComponent(imageUrl)}`, {
        ...
        await sock.sendMessage(m.chat, {
            image: resultBuffer,
            caption: '🎎 *ᴋᴏɴɴɪᴄʜɪᴡᴀ ᴛᴜᴀɴ!* 🎎\n\nᴄʟɪɴɢɢɢ~ ꜰᴏᴛᴏ ᴛᴜᴀɴ ᴜᴅᴀʜ ʙᴇʀᴜʙᴀʜ ᴊᴀᴅɪ ᴀʟᴀ-ᴀʟᴀ ᴊᴇᴘᴀɴɢ ʏᴀɴɢ ᴋᴀᴡᴀɪɪ ᴅᴀɴ ᴇꜱᴛᴇᴛɪᴋ ʙᴀɴɢᴇᴛ! ꜱᴜɢᴏɪɪɪ~ 🌸🍡'
        }, { quoted: m })
