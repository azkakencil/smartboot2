...
const pluginConfig = {
    ...
    alias: ['mekah', 'dimakah'],
    category: 'ai',
    description: 'Ubah foto jadi di mekah',
    usage: '.tomekah (reply/caption foto)',
    example: '.tomekah',
    isOwner: false,
    isPremium: false,
    isGroup: false,
...
const TOMEKAH_URL = 'https://api-faa.my.id/faa/tomekah?url='
...
async function handler(m, { sock }) {
    ...
    if (!fromQuoted && !fromDirect) {
        return m.reply(
            `🕋 ᴛᴏ ᴍᴇᴋᴀʜ\n\n` +
            `> Kirim/reply foto yang mau diubah jadi to mekah\n\n` +
            `\`${m.prefix}tomekah\``
        )
    }
    await m.react('⌛')
    try {
        ...
        const res = await axios.get(`${TOMEKAH_URL}${encodeURIComponent(imageUrl)}`, {
        ...
        await sock.sendMessage(m.chat, {
            image: resultBuffer,
            caption: 'ɪɴɪ ᴛᴜᴀɴ ᴀᴋᴜ ᴜᴅᴀʜ ᴍᴇᴍʙᴜᴀᴛ ꜰᴏᴛᴏ ᴍᴇɴᴊᴀᴅɪ ᴅɪ ᴍᴇᴋᴀʜ'
        }, { quoted: m })
        await m.react('✅')
    } catch (error) {
        await m.react('❌')
