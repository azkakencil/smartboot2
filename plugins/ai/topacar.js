...
const pluginConfig = {
    name: 'topacar',
    alias: ['pacar', 'mygf', 'mybf', 'kekasih'],
    category: 'ai',
    description: 'Ubah foto kamu atau cari tahu versi pacar ideal lewat AI v2',
    usage: '.topacar (reply/caption foto)',
    example: '.topacar',
    isOwner: false,
    isPremium: false,
    isGroup: false,
...
const TOPACAR_URL = 'https://api-faa.my.id/faa/topacarv2?url='
...
async function handler(m, { sock }) {
    ...
    if (!fromQuoted && !fromDirect) {
        return m.reply(
            `❤️ *ᴛᴏ ᴘᴀᴄᴀʀ ᴠ𝟸* ❤️\n\n` +
            `> Mau liat versi pacar idaman Tuan atau bikin foto jadi ala-ala kekasih idola?\n` +
            `> Kirim atau reply fotonya ke sini yuk, Tuan! 🥰\n\n` +
            `\`${m.prefix}topacar\``
        )
    }
    ...
    try {
        ...
        const res = await axios.get(`${TOPACAR_URL}${encodeURIComponent(imageUrl)}`, {
        ...
        await sock.sendMessage(m.chat, {
            image: resultBuffer,
            caption: '💕 *ᴜᴛᴜᴛᴜᴛᴜ~* 💕\n\nᴄʟɪɴɢɢɢ~ ɪɴɪ ᴅɪᴀ ᴛᴜᴀɴ, ʜᴀsɪʟ ꜰᴏᴛᴏ ᴠᴇʀsɪ ᴘᴀᴄᴀʀ ʏᴀɴɢ sᴜᴘᴇʀ ɪᴅᴇᴀʟ ᴅᴀɴ ʙɪᴋɪɴ ʙʟᴜsʜɪɴɢ! ᴊᴀɴɢᴀɴ sᴀᴍᴘᴀɪ ᴘɪɴᴅᴀʜ ᴋᴇ ʟᴀɪɴ ʜᴀᴛɪ ʏᴀ, ᴛᴜᴀɴ~ ᴍᴇʀᴇᴍ sᴇᴅɪᴋɪᴛ ʙᴏʟᴇʜ ɢᴀᴋ ʏᴀ? ᴘᴇɴɢᴇɴ ᴘᴇʟᴜᴋ ᴅᴇʜ ᴇʜ xɪxɪxɪ 🤭✨'
        }, { quoted: m })
