...
const pluginConfig = {
    ...
    alias: ['iqc'],
    category: 'canvas',
    description: 'Membuat gambar quote chat iPhone bertema pink.',
    usage: '.iqc [teks] | [waktu]',
    example: '.iqc Kesendirian adalah teman terbaik ku 😂 | 22.54',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
const BG_LOCAL = join(process.cwd(), 'src', 'cache', 'Iqcbyrin.png'); 
...
const APPLE_EMOJI_JSON_LOCAL = join(process.cwd(), 'src', 'cache', 'emoji-apple-image.json');
...
async function loadAppleEmojiMap() {
    ...
    const cacheDir = dirname(APPLE_EMOJI_JSON_LOCAL);
...
async function ensureAssets() {
    const fontDir = join(process.cwd(), 'src', 'cache', 'fonts');
    ...
    const bgDir = dirname(BG_LOCAL);
    if (!existsSync(bgDir)) await mkdir(bgDir, { recursive: true });
...
async function generateScene(text, time) {
    await ensureAssets();

    const s = { ...state, text: text ?? state.text, time: time };
...
async function handler(m, { sock, text, args }) {
    // 1. Tangkap input lebih kuat (kompatibel dengan parameter bawaan Hisoka/Nexa/dll)
    let inputString = text || (args && args.join(' ')) || '';
    
    // Jika tidak didapat dari parameter 'text', coba ekstrak langsung dari pesan raw
    if (!inputString) {
        const rawMsg = m.text || m.body || m.message?.conversation || m.message?.extendedTextMessage?.text || '';
        inputString = rawMsg.replace(/^\s*[\w\.\?\#\@\-\+\=\%\&\!\/\\]+\s*/, '').trim();
    }

    // 2. Hilangkan argumen 'pink' kalau user ngetik ".iqc pink <teks>"
    if (inputString.toLowerCase() === 'pink') {
        inputString = '';
    } else if (inputString.toLowerCase().startsWith('pink ')) {
        inputString = inputString.substring(5).trim();
    }

    // 3. Ambil teks kalau di-reply
    const quotedText = (m.quoted?.text || m.quoted?.body || m.quoted?.caption || '').trim();
    const autoWIBTime = getCurrentWIB();

    // 4. Pisahkan teks dan jam (jika ada pembatas |)
    let targetText = inputString || quotedText || state.text;
    let targetTime = autoWIBTime;

    if (inputString.includes('|')) {
        const parts = inputString.split('|').map(v => v.trim());
        targetText = parts[0] || quotedText || state.text;
        
        // Kalau jamnya sengaja ditinggal kosong (contoh: "Halo | "), pakai jam otomatis
        targetTime = parts[1] ? parts[1] : autoWIBTime;
    }

    try {
        await m.react('🕕');
        const imgBuffer = await generateScene(targetText, targetTime);
        await sock.sendMessage(m.chat, { image: imgBuffer, caption: `🎨 *iPhone Chat Quote Pink*` }, { quoted: m });
        await m.react('✅');
    } catch (e) {
        console.error(e);
        await m.react('❌');
        m.reply(`❌ *Gagal membuat quote:* ${e.message || e}`);
    }
}
